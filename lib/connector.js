				...
				if (data && data.mode === 'post') {
					...
					if (data.index !== false) {
						...
						_contribution.Attachements.forEach((a, akey)=>{
							log.info(a.Original, akey, data.index, _contributions.Attachements.length)
							if (akey === data.index) {
								_assets[_contribution.Attachements[_contributions.Attachements.length - 1].Original.split('/').pop()] = a.Captions
								_assets[a.Original.split('/').pop()] = [];
							}
							else {
								if (akey < _contributions.Attachements.length - 1) {
