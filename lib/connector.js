module.exports =  function(config, log, slack) {
    ...
    module.connection = new(module.cradle.Connection)(server, port, {
	  ...
	  module.putIssueRokfor = function(group) {
	    ...
	    return new Promise((resolve, reject) => {
	      ...
	      req.send({
	        ...
	        "Forbook": self.api.groups[group].book
	      });
	  ...
	  */
	  ...
	  module.signup = async function(email, group) {
		...
		if (!group || module.api.groups[group] == undefined) {
    ...
    **/
	...
	module.handleAssets = async function(b64mail, password, data) {
		var userdb = await module.loadUserDatabase(b64mail, password);
		if (userdb === false) {
			...
			try {
				...
				// Variant Handling
				if (data && (data.variant !== undefined && data.variant !== false)) {
					
					// 1 GET DATA
					log.info(`  - Start Variant Get`)
					_contribution = await new Promise((resolve, reject) => {
						var req = module.unirest("GET", `${module.api.endpoint}contribution/${data.id}?flat=true`);
						req.headers({
							"content-type": "application/json",
					...
					// Variant Uploads
					if (data && data.mode === 'post') {
					
						let _oldVariantId = _contribution.Attachements[data.index].Captions[2]

						// Upload Variant
						let _newVariantId = module.guidGenerator();
						await new Promise((resolve, reject) => {
							log.info(`  - Start Variant Upload New Id ${_newVariantId}`)

							let payload = {
								"Data": {
									"AttachementsAlternate": {
										'upload': [_newVariantId]
									}
								}
							}
							var upload = data.binary.split(',')[1]; 
							var buf = Buffer.from(upload, 'base64');
							let options = {
								'method': 'POST',
								'url': `${module.api.endpoint}contribution/${data.id}`,
								'headers': {
									"authorization": `Bearer ${module.jwt}`
								},
								formData: {
							...
							module.unirest.request(options, function (error, res) {
								if (error) {
									reject(error);
								} else {
									resolve(res.body);
						...
						log.info(`  - Variant uploaded`)

						// In Place Variant switch
						if (_oldVariantId !== false && _oldVariantId !== undefined && _oldVariantId.trim() !== '') {
							log.info(`  - Start in place`)
							_contribution = await new Promise((resolve, reject) => {
								var req = module.unirest("GET", `${module.api.endpoint}contribution/${data.id}?flat=true`);
								req.headers({
									"content-type": "application/json",
							...
							let _assets = {};
							let _orderedAssets = {};
							_contribution.AttachementsAlternate.forEach((a, akey)=>{
								log.info(`  - ${a.Captions}`)
								if (a.Captions[0] === _oldVariantId) {
									_assets[_contribution.AttachementsAlternate[_contribution.AttachementsAlternate.length - 1].Original.split('/').pop()] = a.Captions[0]
									_orderedAssets[_contribution.AttachementsAlternate[_contribution.AttachementsAlternate.length - 1].Original.split('/').pop()] = a.Captions[0]
									_assets[a.Original.split('/').pop()] = [];
								}
								else {
									if (akey < _contribution.AttachementsAlternate.length - 1) {
										_assets[a.Original.split('/').pop()] = a.Captions[0];
										_orderedAssets[a.Original.split('/').pop()] = a.Captions[0];
									}
								}
							})
							log.info(_assets)
							log.info(_orderedAssets)
							// Delete
							_contribution = await new Promise((resolve, reject) => {
								log.info(`  - Start post`)
								var req = module.unirest("POST", `${module.api.endpoint}contribution/${data.id}`);
								req.headers({
									"authorization": `Bearer ${module.jwt}`,
									"Content-Type": "application/json"
								});
							...
							_contribution = await new Promise((resolve, reject) => {
								...
								let payload = {
									"Data": {
									"AttachementsAlternate": _orderedAssets
									}
								}
								req.send(payload);						
								req.end(function (res) {
							...
							return {error: false, data: _contribution}						
						}
						else {
							log.info(`  - Start adding id ${_newVariantId}`)
							let _final = await new Promise((resolve, reject) => {
								var req = module.unirest("POST", `${module.api.endpoint}contribution/${data.id}`);
								req.headers({
									"authorization": `Bearer ${module.jwt}`,
									"Content-Type": "application/json"
								...
								req.type("json");
								let _assets = {};
								_contribution.Attachements.forEach((a, akey)=>{
									if (akey === data.index) {
										log.info(`  - Compare ${akey} / ${data.index}`)
										_assets[a.Original.split('/').pop()] = [a.Captions[0], a.Captions[1], _newVariantId, a.Captions[3], a.Captions[4]];
									} else {
										_assets[a.Original.split('/').pop()] = a.Captions;
									}
								})
								let payload = {
									"Data": {
									"Attachements": _assets
									}
								}
								req.send(payload);						
								req.end(function (res) {
							...
							return {error: false, data: _final}
						}
						return {error: false, data: _contribution}
					}
					// Variant Deletion
						...
						// Prepeare AltAssets
						let _assets = {};
						let _oldVariantId = data.assets[data.delete].Captions[2];
						_contribution.AttachementsAlternate.forEach((a, akey)=>{
							if (a.Captions[0] === _oldVariantId) {
								_assets[a.Original.split('/').pop()] = [];
							}
						...
						let _final = await new Promise((resolve, reject) => {
							...
							req.type("json");
							let _assets = {};
							data.assets.forEach((a, akey)=>{
								if (a.Captions[2] == _oldVariantId)
									_assets[a.Original.split('/').pop()] = [a.Captions[0], a.Captions[1], '', a.Captions[3], a.Captions[4]];
								else 
									_assets[a.Original.split('/').pop()] = a.Captions;
							})
							let payload = {
					...
					if (data && data.mode === 'post') {
						...
						_contribution = await new Promise((resolve, reject) => {
							...
							let payload = {
								"Data": {
								"Attachements": {
										'upload': ['Edit Caption', `Uploader: ${b64mail}\nTime: ${new Date()}`, ' ', ' ', 'false']
									}
								}
							}
						...
						if (data.index !== false) {
							...
							_contribution.Attachements.forEach((a, akey)=>{
								if (akey === data.index) {
									_assets[_contribution.Attachements[_contribution.Attachements.length - 1].Original.split('/').pop()] = a.Captions
									_orderedAssets[_contribution.Attachements[_contribution.Attachements.length - 1].Original.split('/').pop()] = a.Captions
									_assets[a.Original.split('/').pop()] = [];
								}
								else {
									if (akey < _contribution.Attachements.length - 1) {
										_assets[a.Original.split('/').pop()] = a.Captions;
										_orderedAssets[a.Original.split('/').pop()] = a.Captions;
									}
								}
							})
						...
						if (_contribution.Attachements && _contribution.Attachements.length) {
							_contribution.Attachements.forEach(e => {
								e.Captions[0] = e.Captions[0].replace(/<br \/>/g, '')
								e.Captions[1] = e.Captions[1].replace(/<br \/>/g, '')
								if (e.Captions[3]) {
									e.Captions[3] = e.Captions[3].replace(/<br \/>/g, '')
								}
								e.AttachementsAlternate = false
								if (e.Captions[2] && _contribution.AttachementsAlternate && _contribution.AttachementsAlternate.length) {
									_contribution.AttachementsAlternate.forEach((_alt) => {
										if (_alt.Captions[0] === e.Captions[2]) {
											e.AttachementsAlternate = _alt
										}
									})
								}
							})
						}
						return {error: false, data: _contribution.Attachements || []}
					}
					else if (data && data.mode === 'update' && data.assets) {
						_contribution = await new Promise((resolve, reject) => {
							var req = module.unirest("POST", `${module.api.endpoint}contribution/${data.id}`);
							req.headers({
								"authorization": `Bearer ${module.jwt}`,
					...
					else if (data && data.mode === 'delete' && data.assets) {

						// Prepeare AltAssets
						let _assetsAlternate = {};
						let _oldVariantId = data.assets[data.delete].Captions[2];
						if (typeof _oldVariantId == "string" && _oldVariantId.trim() != '') {
							_contribution = await new Promise((resolve, reject) => {
								var req = module.unirest("GET", `${module.api.endpoint}contribution/${data.id}?flat=true`);
								req.headers({
									"content-type": "application/json",
							...
							_contribution.AttachementsAlternate.forEach((a, akey)=>{
								if (a.Captions[0] === _oldVariantId) {
									_assetsAlternate[a.Original.split('/').pop()] = [];
								}
								else {
									_assetsAlternate[a.Original.split('/').pop()] = a.Captions[0];
								}
							})
							// Delete AltAssets
							await new Promise((resolve, reject) => {
								...
								let payload = {
									"Data": {
										"AttachementsAlternate": _assetsAlternate
									}
								}
								req.send(payload);						
								req.end(function (res) {
		...
		if (userdb === false) {
            ...
            try	{
				let exporters = await new Promise((resolve, reject) => {
				    var req = module.unirest("GET", `${module.api.endpoint}exporter?filter=Book&filterId=${module.api.groups[group].book}`);
				    req.headers({
				      "content-type": "application/json",
				      "authorization": `Bearer ${module.api.rokey}`
				    });
				    req.end(function (res) {
				      if (res.error) {
				        log.error(`Load Exporters Failed: ${res.error}`);
				        reject("Could not load exporters");
				      }
				      else {
					try {
					    resolve(res.body.Exporter.Book[module.api.groups[group].book]);
					}
					catch (err) {
					    resolve([]);
					}
	      ...
	      if (changes.doc.data !== undefined) {
	        if (changes.id.indexOf('options') !== -1) {
	          if (changes.doc.data.Id) {
				...
				try {
					let _literature = "";
					issue.Options.forEach(function(_option) {
						if (_option.key === 'Literature') {
							_literature = _option.value;
						}
					})
					if (_literature !== "") {
						let json = new Cite(_literature)
						let output = json.format('data', {format: 'object'})
						output.forEach(o => { delete o._graph });
						issue.Options.push({
							key: '_csl',
							value: JSON.stringify(output)
						})
						log.info(`Sucessfully converted Literature`);
					}
				} catch (err) {
					log.info(`Error Post Processing Changes ${err}`);
				}
	          ...
	          try {
	            await new Promise((resolve, reject) => {
	              _module.storeContribution(changes, name).then(function(){
					...
					var _chapter = await _module.resolveChapter(changes.doc.data.issue);
	                ...
	                req.send({
	                  ...
	                  "Status": changes.doc.data.status === true ? "Published" : "Draft"
					});
	                req.end(function (res) {
	                  if (res.error) {
	                     reject("PUT FAILED");
	  ...
	  module.storeContribution = function(changes, dbname, id) {
	    ...
	    let payload = {
	      ...
	      "Data": {
			...
			"Settings": changes.doc.data.settings,
	        "_couchDB": changes.id,
