module.exports =  function(config, log) {
	...
	module.addIssue = async function(b64mail, password) {
		...
		if (userdb === false) {
            ...
            let newIssue = false;
            try {
              newIssue = await module.putIssueRokfor();
            } catch (err) {
    ...
    **/
	...
	module.getExporters = async function(b64mail, password) {
		var userdb = await module.loadUserDatabase(b64mail, password);
		if (userdb === false) {
			return false;
		} else {
            try	{
				let exporters = await new Promise((resolve, reject) => {
				    var req = module.unirest("GET", `${module.api.endpoint}exporter?filter=Book&filterId=${module.api.book}`);
				    req.headers({
				      "content-type": "application/json",
				    ...
				    req.end(function (res) {
				      if (res.error) {
				        ...
				        resolve(res.body.Exporter.Book[module.api.book])
				      }
				    });
				});
				return exporters;
	...
	module.storeExport = async function(issueId, exportdata) {
		try {
			let _return = await new Promise((resolve, reject) => {
				var db = module.connection.database(`issue-${issueId}`);
		        db.save(`contribution-${issueId}-exports`, exportdata, function (err, res) {
		          if (err) {
					...
					for (let _file in d.doc.File) {
						var attachmentData = {
						  name: _file + '.pdf',               // something like 'foo.txt'
						  'Content-Type': 'application/pdf' // something like 'text/plain', , 'image/jpeg' etc.
						}
						...
						try {
							await new Promise((resolve, reject) => {
								...
								request(d.doc.File[_file]).pipe(writeStream)
							});
						} catch (err) {
