			...
			try {
				...
				if (data && (data.variant !== undefined && data.variant !== false)) {
					if (data && data.mode === 'post') {
						...
						let _variantIndex = _contribution.Attachements[data.index].Captions[2]
						log.info(`  - Attachement to be modified: ${_variantIndex}`)
						...
						if (_variantIndex !== false && _variantIndex !== undefined && typeof _variantIndex !== 'string') {
							...
							_variantIndex = _contribution.AttachementsVariant && _contribution.AttachementsVariant.length
												? _contribution.AttachementsVariant.length
												: 0
							log.info(`  - Start new variant id ${_variantIndex}`)
							let _final = await new Promise((resolve, reject) => {
								...
								_contribution.Attachements.forEach((a, akey)=>{
									if (akey === data.index) {
										...
										_assets[a.Original.split('/').pop()] = [a.Captions[0], a.Captions[1], _variantIndex];
									} else {
										_assets[a.Original.split('/').pop()] = a.Captions;
									}
								})
